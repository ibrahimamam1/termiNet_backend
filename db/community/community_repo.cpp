...
int CommunityRepository::addNewCommunity(const std::string &name,
                                         std::string &description,
                                         const std::string &iconImage,
                                         const std::string &bannerImage,
  ...
  try {
    ...
    conn.prepare("insert_community",
                 "INSERT INTO communities(community_id, name, description, "
                 "icon_image, banner_image, created_at) "
                 "VALUES ($1, $2, $3, $4, $5, NOW() )");

    tx->exec_prepared("insert_community", id, name, description, iconImage,
                      bannerImage);

    // inser categories in community_categories

    conn.prepare("insert_categories",
...
pqxx::result CommunityRepository::getCommunities(const std::string &filter,
  ...
  try {
    ...
    if (filter == "user_id") {
      conn.prepare("get_comms",
                   "SELECT * FROM communities where community_id IN (select "
                   "community_id from users_communities where user_id = $1)");
...
int CommunityRepository::removeUserFromCommunity(const std::string &user_id,
  ...
  try {
    ...
    pqxx::work tx{conn};
    conn.prepare("remove_user",
                 "DELETE FROM users_communities where user_id = $1 and community_id = $2"
                 );
    pqxx::result res{tx.exec_prepared("remove_user", user_id, community_id)};
    tx.commit();
    errMsg = "No Error";
    ConnectionManager::getInstance()->releaseConnection(conn_index);
    return 200;
